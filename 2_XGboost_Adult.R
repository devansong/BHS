...
setwd("~/Dropbox/CurrentProjects/Connor_Sheep_Survival/data") #set working directory

df <- read.csv("fulldataset.csv") #load csv of chem-survival tab 
str(df) #examine data; make sure things that are numeric are numeric 
df$age <- as.numeric(df$age) # run this line if age is not numeric

survival <- subset(df, AdultDelta == "FALSE" | AdultDelta == "TRUE" ) #subset data which has survival info. 134 observations. 

survival <- subset(survival, Notes != "capture related mort - don't include")
#extract the dataset you want for analysis, calling the new dataset "subdf" 

subdf <- survival[, c("AdultDelta", 
                "pop", 
                "sex", 
                "age", 
                "blood.urea.nitrogen", 
...
subdf <- dummy_cols(subdf, select_columns = 'pop') #create dummy variables for population 
subdf <-subdf[, !(colnames(subdf) %in% c("pop"))] #remove pop.y column 
...
#######Base code for one round of analysis####
...
write.csv(newdf, file="xgboost_results1000_adult.csv")
data <- read.csv("xgboost_results1000_adult.csv") #load csv of xgboost results that you saved earlier
str(data)
...
write.csv(newdf, file="summary_xgboost_1000_adult.csv", row.names=FALSE) 
